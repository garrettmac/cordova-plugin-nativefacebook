...
- (void) graphApi:(CDVInvokedUrlCommand *)command{
    ...
    //    NSString *graphOptions = [command argumentAtIndex:2];
    //
    //    NSDictionary *options = [command.arguments objectAtIndex:0];
    //    NSLog(@"Graph JSON Options = %@", options);
    //  NSString *graphPath = [NSString stringWithFormat:@"/me"];
    NSMutableDictionary *options = [[command.arguments objectAtIndex:0] mutableCopy];
    NSString*graphPath = options[@"url"];
    NSString*graphMethod = options[@"method"];
    
    // NSString*graphMethode=  [NSString stringWithFormat: @"Status: %@", (graphMethod ? @"Approved" : @"Rejected")]
    //        if(!options[@"method"]){
    //        NSString*graphMethod = [NSString stringWithFormat:@"GET"];
    //        }else {
    //            NSString*graphMethod = options[@"method"];
    //            return;
    //        }
    ...
    FBSDKGraphRequestHandler graphHandler = ^(FBSDKGraphRequestConnection *connection, id result, NSError *error) {
        ...
        CDVPluginResult* pluginResult;
        NSLog(@"Graph pluginResult = %@", pluginResult);
        if (error) {
            NSLog(@"Error: graph call = %@", error);
            NSString *message = error.userInfo[FBSDKErrorLocalizedDescriptionKey] ?:[NSString stringWithFormat: @"There was an error making the graph call: %@", error ];
            //@"There was an error making the graph call.";
            //+ [NSError stringWithFormat: @"Error: %@", error ];
    ...
    NSString *paramKey;
    //NSDictionary *params = [responseString options[@"params"]];
    paramKey=[params valueForKey:@"fields"];
    NSString *url = [params objectForKey:@"fields"];
    NSString *myImage = [params objectForKey:@"image"];
    NSLog(@"paramKey = %@", paramKey);
    NSLog(@"url = %@",url );
    //NSLog(@" = %@", );
    NSLog(@"myImage = %@", myImage);
    
    //@{@"url": @"{image-url}",};myImageURL
    NSLog(@"Graph JSON options = %@", options);
    NSLog(@"Graph JSON options = %@", options);
    NSLog(@"Graph JSON method = %@", graphMethod);
    NSLog(@"Graph JSON params = %@", params);
    
    NSLog(@"Graph Path = %@", graphPath);
    //NSLog(@"Graph JSON options = %@", params);
    
    FBSDKGraphRequest *request = [[FBSDKGraphRequest alloc]
                                  initWithGraphPath:(graphPath)?graphPath:@"/me"
                                  parameters:(params)?params:nil
                                  //parameters:@{@"fields": @""}
                                  HTTPMethod:(graphMethod)?graphMethod:@"GET"];
